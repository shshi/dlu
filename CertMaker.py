...
class Graduate:
	...
	def FS(self,doc,row,newPage):
		...
		BanckAcc=row[9]
		SwiftCode=row[10]
		body_txt='''This is to certify that %s%s (passport No. %s), %s student of %s batch of Dali University. The fee structure for academic year 2018 – 2019 is as per below. The exchange rate of US Dollar into Chinese Yuan for this academic year is 6.41 (according to the exchange rate provided by Bank of China on May 31th, 2018).'''%(Sex,Name,PPNo,PG,Grade)
		...
		RegFee,MedFee,AccmdFee,DineFee,PocketFee,TranspFee,InsFee="3000RMB","18000RMB","2200RMB","3450RMB","16500RMB","6850RMB","600RMB"
		TotalAmount=str(int(RegFee.replace('RMB',''))+int(MedFee.replace('RMB',''))+int(AccmdFee.replace('RMB',''))+int(DineFee.replace('RMB',''))+int(PocketFee.replace('RMB',''))+int(TranspFee.replace('RMB',''))+int(InsFee.replace('RMB','')))+'RMB'
		...
		Note_Bank='''Note: Those contents with asterisk are suggested amount Total fee for this academic year is 26800 RMB and %s personal expenses including insurance and visa are 23800 RMB. %s Tuition fee and personal expenses can be sent to %s personal account. Dali University has no any issue sending Tuition fee in %s personal account.\nAccount Name: AAMIR AKRAM\nDeposit Bank: %s\nBank Account No: %s\nSwift Code: %s'''%(HeShe,'H'+HeShe[1:],HeShe,HeShe,Bank,BanckAcc,SwiftCode)
		
		NB=doc.add_paragraph()
		NB.paragraph_format.line_spacing = docx.shared.Pt(20)
